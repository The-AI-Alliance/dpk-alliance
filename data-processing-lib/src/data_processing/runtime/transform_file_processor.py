...
class AbstractTransformFileProcessor:
    """
    This is the base class implementing processing of a single binary file
    """
    ...
    def __init__(
        ...
        if data_access_factory[0] is None or data_access_factory[1] is None:
            self.logger.error(
                "Transform file processor: data access factory is not specified"
            )
    ...
    def process_file(self, f_name: str) -> None:
        ...
        if not self.is_folder:
            ...
            if filedata is None:
                self.logger.warning(
                    f"File read resulted in None for {f_name}. Returning."
                )
        ...
        try:
            ...
            if not self.is_folder:
                ...
                out_files, stats = self.transform.transform_binary(
                    file_name=f_name, byte_array=filedata
                )
        ...
        # Process unrecoverable exceptions
            ...
            self.logger.warning(
                f"Transform has thrown unrecoverable exception processing file {f_name}. Exiting..."
            )
            raise UnrecoverableException
        # Process other exceptions
        except Exception as _:
            self.logger.warning(
                f"Exception processing file {f_name}: {traceback.format_exc()}"
            )
    ...
    def _submit_file(
        self, t_start: float, out_files: list[tuple[bytes, str]], stats: dict[str, Any]
    ) -> None:
        """
        ...
        match len(out_files):
            ...
            case 1:
                ...
                if self.is_folder:
                    ...
                    output_name = self.data_access.get_output_location(
            ...
            case _:
                ...
                file_sizes = 0
                output_file_name = self.data_access.get_output_location(
                ...
                for index in range(count):
                    ...
                    file_sizes += len(dt)
                    save_res, retries = self.data_access_output.save_file(
                        path=output_name_indexed, data=dt
                    )
                    if retries > 0:
                        self._publish_stats({"data access retries": retries})
                    if save_res is None:
                        self.logger.warning(
                            f"Failed to write file {output_name_indexed}"
                        )
                        self._publish_stats({"failed_writes": 1})
                        break
    ...
    def _publish_stats(self, stats: dict[str, Any]) -> None:
        """
        execution statistics publishing
