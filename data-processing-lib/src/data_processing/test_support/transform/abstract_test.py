...
_percent_parquet_size_diff_allowed = (
    0.03  # 3% seems like the minimum, but somewhat arbitrary
)


def get_tables_in_folder(path: str) -> list[pa.Table]:
    """
    Get  list of Tables loaded from the parquet files in the given directory.  The returned
    list is sorted lexicographically by the name of the file.
    :param path:
    :return:
    """
    dal = DataAccessLocal()
    files, _ = dal.get_folder_files(path, extensions=[".parquet"])
    return [TransformUtils.convert_binary_to_arrow(data) for data in files.values()]


def get_files_in_folder(
    path: str, ext: str, return_data: bool = True
) -> dict[str, bytes]:
    """
    ...
    """
    dal = DataAccessLocal()
    files, _ = dal.get_folder_files(path, extensions=[ext], return_data=return_data)
    return files


class AbstractTest:
    """
    This class enables a test convention for pytest in which subclasses define the tests and a method to install
    the pytest test fixtures for the tests.  This class's primary responsibility is to use the pytest hook
    method pytest_generate_tests() to have the subclass install the fixtures using _install_test_fixtures().
    ...
    def pytest_generate_tests(metafunc):
        ...
        test_instance = (
            metafunc.cls()
        )  # Create the instance of the test class being used.
    ...
    def validate_expected_tables(
        cls, table_list: list[pa.Table], expected_table_list: list[pa.Table]
    ):
        """
        ...
        assert expected_table_list is not None, (
            "Test misconfigured: expected table list is None"
        )

        l1 = len(table_list)
        l2 = len(expected_table_list)
        assert l1 == l2, (
            f"Number of transformed tables ({l1}) is not the expected number ({l2})"
        )
        for i in range(l1):
            t1 = table_list[i]
            ...
            rows2 = t2.num_rows
            assert rows1 == rows2, (
                f"Number of rows in table #{i} ({rows1}) does not match expected number ({rows2})"
            )
    ...
    def validate_expected_row(
        ...
        assert expected_row.num_rows == 1, (
        ...
        if test_row != expected_row:
            ...
            for i in range(num_columns):
                ...
                if test_column != expected_column:
                    ...
                    if not is_float:
                        # It's NOT a float, so do a normal compare
    ...
    def validate_expected_files(
        ...
        assert expected_files_list is not None, (
            "Test misconfigured: expected table list is None"
        )
        ...
        assert l1 == l2, (
            f"Number of transformed files ({l1}) is not the expected number ({l2})"
        )
        for i in range(l1):
            f1 = files_list[i]
    ...
    def validate_expected_metadata_lists(
        ...
        assert len(metadata) == elen, (
            f"Number of metadata dictionaries not the expected of {elen}"
        )
    ...
    def validate_expected_metadata(
        cls, metadata: dict[str, float], expected_metadata: dict[str, float]
    ):
        """
    ...
    def validate_directory_contents(
        cls, directory: str, expected_dir: str, drop_columns: list[str] = []
    ):
        """
        ...
        len_funny = len(
            dir_cmp.funny_files
        )  # Do this earlier than an assert for debugging reasons.
        assert len_funny == 0, (
            f"Files that could compare, but couldn't be read for some reason: {dir_cmp.funny_files}"
        )
        assert len(dir_cmp.common_funny) == 0, (
            f"Types of the following files don't match: {dir_cmp.common_funny}"
        )
        assert len(dir_cmp.right_only) == 0, (
            f"Files found only in expected output directory: {dir_cmp.right_only}"
        )
        assert len(dir_cmp.left_only) == 0, (
            f"Files missing in test output directory: {dir_cmp.left_only}"
        )
        ...
        if failed:
            cls.__confirm_diffs(
                directory, expected_dir, dir_cmp.diff_files, "/tmp", drop_columns
            )

        # Traverse into the sub dirs since dircmp doesn't seem to do that.
        sub_dirs = [f.name for f in os.scandir(expected_dir) if f.is_dir()]
    ...
    def _validate_table_files(
        cls, parquet1: str, parquet2: str, drop_columns: list[str] = []
    ):
        da = DataAccessLocal()
        filedata, _ = da.get_file(parquet1)
        t1 = TransformUtils.convert_binary_to_arrow(filedata)
        filedata, _ = da.get_file(parquet2)
        t2 = TransformUtils.convert_binary_to_arrow(filedata)
    ...
    def __confirm_diffs(
        ...
        for file in diff_files:
            ...
            if "parquet" in file:
                ...
                # so for these, do the in memory comparison.
                try:
