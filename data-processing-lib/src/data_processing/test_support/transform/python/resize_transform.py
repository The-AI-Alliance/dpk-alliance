...
class ResizeTransform(AbstractTableTransform):
    ...
    def __init__(self, config: dict[str, Any]):
        ...
        if self.max_rows_per_table <= 0 and self.max_bytes_per_table <= 0:
            raise ValueError(
                "Neither max rows per table nor max table size are defined"
            )
        if self.max_rows_per_table > 0 and self.max_bytes_per_table > 0:
            raise ValueError(
                "Both max rows per table and max table size are defined. Only one should be present"
            )
    ...
    def transform(
        ...
        if self.buffer is not None:
            try:
                ...
                self.logger.warning(
                    f"table in {file_name} can't be merged with the buffer"
                )
