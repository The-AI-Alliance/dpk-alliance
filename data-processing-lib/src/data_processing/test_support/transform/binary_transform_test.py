...
from data_processing.test_support.transform import AbstractTest
...
class AbstractBinaryTransformTest(AbstractTest):
    ...
    Generic tests are provided here, and subclasses must implement the _get*_fixture() method(s)
    ...
    def _install_test_fixtures(self, metafunc):
        ...
        if (
            ...
            # Let the subclass define the specific tests and test data for the transform under test.
    ...
    def test_transform(
        ...
        metadata) with the expected values as given.  The inputs are provided by the subclass definition of
        ...
        for in_file in in_binary_list:
            files_list, metadata = transform.transform_binary(
                file_name=in_file[0], byte_array=in_file[1]
            )
        ...
        AbstractBinaryTransformTest.validate_expected_files(
            all_files_list, expected_binary_list
        )
        AbstractBinaryTransformTest.validate_expected_metadata_lists(
            all_metadata_list, expected_metadata_list
        )
