...
class TestInit:
    ...
    dal = DataAccessLocal(path_dict, checkpoint=True, m_files=-1)
...
class TestGetFilesFolder(TestInit):
    def test_empty_directory(self):
        ...
        result = self.dal.get_files_folder(
            path=directory, files_to_use=None, cm_files=0
        )
        os.rmdir(directory)
    ...
    def test_single_file(self):
        ...
        file_path.touch()
        result = self.dal.get_files_folder(
            path=str(directory), files_to_use=None, cm_files=0
        )
        os.remove(file_path)
        os.rmdir(directory)
        assert result == (
    ...
    def test_multiple_files(self):
        ...
        file_list, size_dict, retries = self.dal.get_files_folder(
            path=str(directory), files_to_use=None, cm_files=0
        )
    ...
    def test_non_parquet_file(self):
        ...
        file_path.touch()
        result = self.dal.get_files_folder(
            path=str(directory), files_to_use=[".parquet"], cm_files=0
        )
        os.remove(file_path)
        os.rmdir(directory)
        assert result == ([], self.size_stat_dict_empty, 0)
...
class TestGetInputFiles(TestInit):
    ...
    def test_empty_directories(self):
        ...
        result = self.dal._get_input_files(
            str(input_path), str(output_path), cm_files=0
        )
    ...
    def test_multiple_missing_files(self):
        ...
        file_list, size_dict, retries = self.dal._get_input_files(
            str(input_path), str(output_path), cm_files=0
        )
        ...
        expected_result = (
            [
                str(file.absolute())
                for file in input_files
                if str(file.name) != str(output_file.name)
            ],
            self.size_stat_dict,
        )
    ...
    def test_non_existent_directory(self):
        ...
        with patch("pathlib.Path.rglob") as mock_rglob:
            mock_rglob.return_value = []
            result = self.dal._get_input_files(
                str(nonexistent_input_path), str(output_path), cm_files=0
            )
            self.cleanup(directories_to_remove=[input_path, output_path])
            assert result == ([], self.size_stat_dict_empty, 0)
...
class TestGetFilesToProcess(TestInit):
    ...
    def test_missing_files(self):
        ...
        Test the function with one file missing in the output directory.
        """
        # create the data set subdirectories
    ...
    def multiple_missing_files_cleanup(
        ...
        directories_to_remove = [in_path_1, out_path_1, in_path_2, out_path_2]
        self.cleanup(
            directories_to_remove=directories_to_remove, files_to_remove=files_to_remove
        )
    ...
    def test_multiple_missing_files(self):
        ...
        expected_result = (
            ...
            + [
                str(file.absolute())
                for file in in_files_2
                if str(file.name) != str(out_file_2.name)
            ],
            self.size_stat_dict_1,
        )
        ...
        expected_result = (
            [str(file.absolute()) for file in in_files_1]
            + [str(file.absolute()) for file in in_files_2],
            self.size_stat_dict_1,
        )
        self.multiple_missing_files_cleanup(
            in_files_1,
...
class TestGetOutputLocation(TestInit):
    def test_get_output_location(self):
        ...
        assert out_path == os.path.join(
            self.dal.output_folder, "path", "to", "f.parquet"
        )
        ...
        in_path = os.path.abspath(
            os.path.join("../..", "input_guf", "path", "to", "f.parquet")
        )
...
class TestSaveJobMetadata(TestInit):
    ...
    def test_save_job_metadata(self):
        ...
        assert metadata_file_path == os.path.join(
            self.dal.output_folder, "metadata.json"
        )
        assert (
            len(metadata_dict) == 8
            and metadata_dict.get("source", {}).get("name", "") == self.dal.input_folder
            and metadata_dict.get("target", {}).get("name", "")
            == self.dal.output_folder
        )
...
class TestGetFolderFiles(TestInit):
    ...
    def test_multiple_extensions(self):
        ...
        assert (
            contents_txt_pdf[self.pdf_file_path][:4] == b"\x25\x50\x44\x46"
        )  # Check PDF header
